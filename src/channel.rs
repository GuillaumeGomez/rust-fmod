...
impl Channel {
	...
	pub fn set_position(&self, position : uint, postype : uint) -> FMOD_RESULT {
		unsafe { ffi::FMOD_Channel_SetPosition(self.channel, position as u32, postype as ffi::FMOD_TIMEUNIT) }
	}

	pub fn get_position(&self, postype : uint) -> Result<uint, FMOD_RESULT> {
		let t = 0u32;

		match unsafe { ffi::FMOD_Channel_GetPosition(self.channel, &t, postype as ffi::FMOD_TIMEUNIT) } {
